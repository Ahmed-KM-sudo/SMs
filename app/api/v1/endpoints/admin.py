...
from app.api.v1.schemas import user as user_schema, activity_log as activity_log_schema
...
@router.get("/users/{user_id}", response_model=user_schema.User, tags=["admin-users"])
def read_user(
    user_id: int,
    db: Session = Depends(get_db),
    current_admin: Agent = Depends(get_current_active_admin),
